...
// Generate initial pieces for SSR (deterministic)
const generateInitialPieces = (count = 3) => {
  // Use a fixed sequence for initial SSR render to avoid hydration mismatch
  const initialTypes = ['I', 'O', 'T'] // Fixed sequence for SSR
  return initialTypes.slice(0, count).map(type => TETROMINOS[type])
}

export default function Tetris() {
  const [board, setBoard] = useState<(number | string)[][]>(createEmptyBoard())
  const [currentPiece, setCurrentPiece] = useState<Piece | null>(null)
  const [nextPieces, setNextPieces] = useState<Tetromino[]>(generateInitialPieces())
  ...
  // Initialize next pieces on client side only to avoid hydration mismatch
  useEffect(() => {
  ...
  const checkCollision = useCallback(
    (x: number, y: number, shape: number[][], currentBoard = board) => {
  ...
  const checkGameOver = useCallback((currentBoard: (number | string)[][]) => {
  ...
  const clearLines = useCallback(
    (newBoard: (number | string)[][]) => {
      const linesCleared = []
      const updatedBoard: (number | string)[][] = []
  ...
  useEffect(() => {
    if (audioRef.current) {
      ...
      if (!gameOver && isMusicPlaying && !isPaused && hasUserInteracted) {
        audioRef.current.play().catch((error: any) => console.error("Audio playback failed:", error))
      } else {
        audioRef.current.pause()
      }
    }
  }, [gameOver, isMusicPlaying, isPaused, hasUserInteracted])
  ...
  const renderPiece = (tetromino: Tetromino | null, size = 20) => {
  ...
  const toggleMusic = () => {
    if (!hasUserInteracted && !isMusicPlaying) {
      // If user hasn't interacted yet and trying to turn music on, just enable it
      // The useEffect will handle playing when interaction occurs
